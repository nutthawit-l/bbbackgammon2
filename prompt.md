...
- **Frontend App (`apps/frontend`):** Built with **React** and **Vite**. Features **PixiJS (v8+)** inside an isolated canvas sandbox for rendering the board, checkers, and animations.
...
- React manages application views (menus, overlays, UI states mapped from Figma).
...
3. Generate a clean React application scaffolding inside `apps/frontend` using Vite.
4. Setup a `theme.ts` file structured to easily accept exact hex codes, padding, and layout aspect ratios from my Figma file.
5. Create a specialized React component embedding a PixiJS (v8+) application context.
6. Use PixiJS vector graphics primitives (`Graphics`) to dynamically draw a responsive, beautiful, empty board utilizing your core constants and theme configurations:
